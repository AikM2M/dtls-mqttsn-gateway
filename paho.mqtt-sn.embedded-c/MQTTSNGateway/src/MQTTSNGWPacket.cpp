...
int MQTTSNPacket::unicast2(SSL* _ssl, SensorNetwork* network, SensorNetAddress* sendTo)
{
	return network->unicast2(_ssl, _buf, _bufLen, sendTo);
}
...
int MQTTSNPacket::recv(SensorNetwork* network, SSL* ssl)
{
	uint8_t buf[MQTTSNGW_MAX_PACKET_SIZE];
	int len = network->read(ssl, (uint8_t*) buf, MQTTSNGW_MAX_PACKET_SIZE);
	if (len > 1)
	{
