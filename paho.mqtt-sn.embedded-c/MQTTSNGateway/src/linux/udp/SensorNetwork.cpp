...
/* Initializing the following parameters:
 	const char* caFILE; 		{path to the stored ca certificate file}
	const char* caKEY;  		{path to the stored ca private key}
	const char* srvFILE;		{path to the stored ca signed server certificate}
	const char* srvKEY;			{path to the stored server private key}
	const char* pskKEY;			{pre-shared symmetric ssl/tls session key in hex}
	const char* pskIDENTITY;	{pre-shared symmetric ssl/tls session key identifier}
	const char* pskHINT;		{hint associated with the pre-shared symmetric key}
	int mode;					{mode of security; '0' for DTLS-PSK and '1' for DTLS-CERT, '1' default}
	int secure;					{flag of security; '0' for no-DTLS and '1' for DTLS-over-UDP, '0' default}
	int cookie_initialized;		{cookie initializing flag; '0' if not initialized, '1' otherwise}
 */
...
{
	//return SSL_write(ssl, payload, payloadLength);
}

int SensorNetwork::unicast2(SSL* _ssl, const uint8_t* payload, uint16_t payloadLength, SensorNetAddress* sendToAddr)
{
	return SSL_write(_ssl, payload, payloadLength);
}
...
int SensorNetwork::read(SSL* _ssl, uint8_t* buf, uint16_t bufLen)
{
	return SSL_read(_ssl, buf, bufLen);
}
...
/* Module 01: 
 * UDP Port Opening Module 
	printf("[.] Opening UDP Port ");
	int rc;
	if ((rc = UDPPort::open(ip.c_str(), multicastPortNo, unicastPortNo)) != 0) 
		return -1;
	else {
		this->sockfd = this->get_sockfdUnicast();
		return rc;
	} */
}
...
int UDPPort::get_sockfdUnicast() {
	return _sockfdUnicast;
}
