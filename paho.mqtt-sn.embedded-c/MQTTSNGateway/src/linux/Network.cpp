 ...
 **************************************************************************************/
/************************************************************************************
 * Contribution:	Support for TLS over TCP with Pre-shared key is added 
...
// TLS PSK DATA SECTION
const char  *pskID_client = "AE321_LOCK@in.provider.com";
char 		*pskKEY_client = "0504030201";
                                  ...
                                  unsigned int max_psk_len)
{
 ...
 //   if (c_debug)
     ...
     */
    ret = BIO_snprintf(identity, max_identity_len, "%s", pskID_client);
    ...
    key = OPENSSL_hexstr2buf(pskKEY_client, &key_len);
    if (key == NULL) {
...
if (TlsPskEnable == true) {
		/* Use Psk & it's Psk-Hint */
		SSL_CTX_set_psk_client_callback(_ctx, &psk_client_cb);
}	
		if (! TCPStack::isValid())
		{
			if ( !TCPStack::connect(host, port) )
