/************************************************************************************
 * Contribution:	SSI information is fetched from the client list to send packet 
 * 					over appropriate 'ssl' session
...
void ClientSendTask::run()
{
	...
	ClientList* _clientList = _gateway->getClientList();
	int rc = 0;
	...
	while (true)
	{
		...
		if ((ev->getEventType() == EtStop) || (_gateway->StopGateway == true))
		{
		...
		else if (ev->getEventType() == EtSensornetSend)
		{
			packet = ev->getMQTTSNPacket();
			log(client, packet);
			rc = _sensorNetwork->unicast2(ev->getClient()->getClientSSL(), packet->getbuf(), packet->getbufLen(), ev->getSensorNetAddress()); // Added 
		}
