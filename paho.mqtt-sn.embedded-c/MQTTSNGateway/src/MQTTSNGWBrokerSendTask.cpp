...
void BrokerSendTask::run()
{
	...
	while (true)
	{
		...
		if ( ev->getEventType() == EtBrokerSend)
		{
			...
			/* Update the Flags via reading config file */
			client->getNetwork()->TlsPskEnable	= _gateway->getGatewayParams().GwTlsPskEnable;
			client->getNetwork()->TlsCertEnable = _gateway->getGatewayParams().GwTlsCertEnable;
