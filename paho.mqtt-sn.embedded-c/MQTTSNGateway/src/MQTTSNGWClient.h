/************************************************************************************
 * Contribution:	Additional information of SSI and thread-id is added into the 
 * 					existing structure 'MQTTSNGWClient'
...
public:
    ...
    void setClientSSL(SSL* _ssl) { ssl = _ssl; }; // Added
    SSL* getClientSSL() { return ssl; };          // Added
    ...
    pthread_t get_tid() { return _cltTHD; };
    void set_tid (pthread_t _tid) { _cltTHD = _tid; };
...
private:
    ...
    SSL* ssl;                           // ssl
    pthread_t _cltTHD;                  // attached thread id
