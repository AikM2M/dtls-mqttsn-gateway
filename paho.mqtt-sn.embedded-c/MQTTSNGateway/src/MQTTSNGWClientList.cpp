...
/*=====================================
 Class ClientList
 =====================================*/
const char* common_topic = "AsimChuddo";
...
pthread_t ClientList::getcltThreadID(MQTTSNString* clientId)
{
    _mutex.lock();
    Client* client = _firstClient;
    const char* clID =clientId->cstring;
...
int ClientList::print_list() {
    Client* client = _firstClient;
    while (client != nullptr)
    {
	printf("%s ", (const char*) client->getClientId());
        client = client->_nextClient;
    }
    printf("\n");
    return 0;
...
Client* ClientList::createClient2(SensorNetAddress* addr, MQTTSNString* clientId, int type, SSL* ssl, pthread_t _tid)
{
	return createClient2(addr, clientId, false, false, type, ssl, _tid);
}
...
Client* ClientList::createClient2(SensorNetAddress* addr, MQTTSNString* clientId, bool unstableLine, bool secure, int type, SSL* ssl, pthread_t _tid)
{
