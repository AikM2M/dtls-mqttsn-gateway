 ...
 * Contribution:	Attribute for Secure Session Information (SSI) is added into the 
 * 					main packet stream to send and receive packets to Sensor Networks
 * 					using 'ssl'
...
public:
	...
	int unicast2(SSL* _ssl, SensorNetwork* network, SensorNetAddress* sendTo);
	int broadcast(SensorNetwork* network);
	int recv(SensorNetwork* network, SSL* ssl);
	...
	int getbufLen() { return _bufLen; }; 
	void setbuf(unsigned char* buf) { _buf = buf; };
	void setbufLen(int len) { _bufLen = len; }; 
